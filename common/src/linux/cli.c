...
static void cli_putchar(struct cli *cli, char ch, bool is_last)
{
    if (cli->put_char) {
#if CLI_SERIAL_XLATE
        if (ch == '\n')
...
static void cli_puts(struct cli *cli, const char *s)
{
    for (; *s; s++)
        cli_putchar(cli, *s, s[1] == '\0');
}

static void cli_reset_line(struct cli *cli)
{
    cli->len = 0;
    cli->cursor = 0;
    cli->counter = 0;
...
void cli_init(struct cli *cli, const char *prompt,
...
static void cli_ansi(struct cli *cli, int n, char code)
{
    char buffer[5] = {'\x1b', '[', '0' + (n % 10), code, '\0'};
    cli_puts(cli, buffer);
}

static void term_cursor_back(struct cli *cli, int n)
{
    while (n > 0) {
        int count = n > 9 ? 9 : n;
...
static void term_cursor_fwd(struct cli *cli, int n)
{
    while (n > 0) {
        int count = n > 9 ? 9 : n;
...
#if CLI_HISTORY_LEN
static void term_backspace(struct cli *cli, int n)
{
...
static const char *cli_get_history_search(struct cli *cli)
{
    for (int i = 0;; i++) {
        const char *h = cli_get_history(cli, i);
        if (!h)
            return NULL;
...
static void cli_insert_default_char(struct cli *cli,
                                             char ch)
{
    ...
    if (cli->searching) {
        ...
        const char *h = cli_get_history_search(cli);
        if (h)
            cli_puts(cli, h);
...
#if CLI_HISTORY_LEN
static void cli_extend_history(struct cli *cli)
{
...
static void cli_stop_search(struct cli *cli, bool print)
{
    const char *h = cli_get_history_search(cli);
    if (h) {
    ...
    if (cli->have_csi) {
        if (ch >= '0' && ch <= '9' && cli->counter < 100) {
            ...
            case 'A': { // up arrow
                ...
                const char *line =
                    cli_get_history(cli, cli->history_pos + 1);
                if (line) {
                    int len = strlen(line);
                    cli->history_pos++;
        ...
        case '[':
            if (cli->have_escape)
                ...
                cli_insert_default_char(cli, ch);
            break;
#if CLI_SERIAL_XLATE
        case '\r':
...
#if CLI_HISTORY_LEN
        if (cli->searching)
            cli_stop_search(cli, false);
        cli_extend_history(cli);
#endif
        cli_reset_line(cli);
    }
